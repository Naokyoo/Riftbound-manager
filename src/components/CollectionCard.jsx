...
const CollectionCard = ({ card, onUpdateQuantity, onRemove, onClick }) => {
    const [isEditing, setIsEditing] = useState(false);
    const [tempQuantity, setTempQuantity] = useState(card.quantity);
    ...
    return (
        <div className={`bg-slate-800/90 rounded-xl overflow-hidden border-2 ${rarityBorders[rarity]} shadow-xl relative group`}>
            {/* Badge de quantité */}
            <div className="absolute top-2 right-2 z-10">
                {isEditing ? (
                    <div className="flex items-center gap-1 bg-slate-900/90 rounded-lg p-1">
                        <input
                            type="number"
                            min="0"
                            value={tempQuantity}
                            onChange={(e) => setTempQuantity(e.target.value)}
                            className="w-14 px-2 py-1 bg-slate-700 text-white text-sm rounded border border-amber-500/50 focus:outline-none"
                            autoFocus
                        />
                        <button
                    ...
                    <button
                        ...
                        className="bg-gradient-to-br from-amber-500 to-amber-600 text-white font-bold px-3 py-1 rounded-full text-sm shadow-lg hover:from-amber-600 hover:to-amber-700 transition"
                    >
                        x{card.quantity}
                    </button>
                )}
            ...
            <button
                ...
                className="absolute top-2 left-2 z-10 bg-red-600/90 text-white w-8 h-8 rounded-full opacity-0 group-hover:opacity-100 transition hover:bg-red-700"
            ...
            <div className="px-4 pb-4 flex gap-2">
                <button
                    ...
                    className="flex-1 bg-green-600/20 text-green-400 py-2 rounded hover:bg-green-600/30 transition text-sm font-semibold"
                >
                    + Ajouter
                </button>
                <button
                    onClick={() => onUpdateQuantity(card.id, Math.max(0, card.quantity - 1))}
                    className="flex-1 bg-red-600/20 text-red-400 py-2 rounded hover:bg-red-600/30 transition text-sm font-semibold"
                >
                    - Retirer
                </button>
            </div>
